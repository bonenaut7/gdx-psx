 ...
 *  
 *  <b>WARNING! This have a very critical problem. Resolution change cannot be done without recompiling shader because it will cause problems with calculations!</b>
 *  
 *  @author fxgaming (FXG)
 */
public class ShaderTransformer {
	public static float DEFAULT_DOWNSCALE_FACTOR = 2.0f;
	...
	protected String injectVertexShader() {
		...
		if (matcher.find()) {
			...
			switch (this.transformType) {
				...
				default: stringBuilder.append("} ").append(vertexShaderParts[1]);
			}
			return stringBuilder.toString();
		} else System.out.println("[GDX-PSX] Can\'t patch shaderProvider because shaders not matching pattern.");
