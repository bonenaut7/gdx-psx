...
float ditherColorChannel(float color, float ditherThreshold, float ditherStep) { 
	float _distance = mod(color, ditherStep);
	float _baseValue = floor(color / ditherStep) * ditherStep;
	return mix(_baseValue, _baseValue + ditherStep, step(ditherThreshold, _distance / ditherStep - 0.001));
}
...
float channelError(float color, float colorMin, float colorMax) {
	float _range = abs(colorMin - colorMax);
	float _aRange = abs(color - colorMin);
	return _aRange / _range;
}

float ditheredChannel(float error, vec2 ditherBlockUV) {
	float _pattern = texture2D(u_ditherTexture, ditherBlockUV).r;
	if (error > _pattern) {
		return 1.0;
	} else {
		return 0.0;
...
void main() {
	...
	vec2 resolution = vec2(u_resolution[0] * (1.0 / u_resolution[1]), u_resolution[0] * (1.0 / u_resolution[2]));
	if (u_flags[1] > 0.0) {
		texCoords = vec2(resolution.x * floor(v_texCoords.x / resolution.x), resolution.y * floor(v_texCoords.y / resolution.y));
	}

	//color-depth
	vec4 fragmentColor = texture2D(u_texture, texCoords);
	vec3 colorDepth = vec3(256.0);
	if (u_flags[2] > 0.0) {
		colorDepth = vec3(u_colorDepth[0], u_colorDepth[1], u_colorDepth[2]);
		vec3 normColorDepth = 1.0 / max(colorDepth, 1.0);
		fragmentColor.r = ditherColorChannel(fragmentColor.r, 0.5, normColorDepth.r);
		fragmentColor.g = ditherColorChannel(fragmentColor.g, 0.5, normColorDepth.g);
		fragmentColor.b = ditherColorChannel(fragmentColor.b, 0.5, normColorDepth.b);
	}
	...
	if (u_flags[0] > 0.0 && gl_FragColor.a < 0.5) { discard; }
}