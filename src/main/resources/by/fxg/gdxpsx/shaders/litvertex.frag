...
void main() {
	...
	//[SHADOWMAP & APPLY OF LIGHTING]================================================================
		...
		gl_FragColor.rgb = diffuse.rgb + emissive.rgb;
				...
				vec3 specular = applyLUT(texture2D(u_specularTexture, v_specularUV _texAfMap), u_specularLUT).rgb * u_specularColor.rgb * v_lightSpecular;
			#else
				vec3 specular = texture2D(u_specularTexture, v_specularUV _texAfMap).rgb * u_specularColor.rgb * v_lightSpecular;
			#endif //LUT
		#elif defined(specularTextureFlag)
			#if defined(LUTMappingFlag) && defined(specularLUTFlag)
				vec3 specular = applyLUT(texture2D(u_specularTexture, v_specularUV _texAfMap), u_specularLUT).rgb * v_lightSpecular;
			#else
				vec3 specular = texture2D(u_specularTexture, v_specularUV _texAfMap).rgb * v_lightSpecular;
			#endif //LUT
