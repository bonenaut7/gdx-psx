...
void main() {
	...
	//[LIGHTING]=====================================================================================
			...
			for (int i = 0; i < numDirectionalLights; i++) {
				...
				#ifdef specularFlag
					float halfDotView = max(0.0, dot(normal, normalize(lightDir + viewVector)));
			...
			for (int i = 0; i < numSpotLights; i++) {
				...
				if (spotFactor > u_spotLights[i].cutoffAngle) {
					...
					v_lightDiffuse += value * (1.0 - (1.0 - spotFactor) * 1.0 / (1.0 - u_spotLights[i].cutoffAngle));
					#ifdef specularFlag
						float halfDotView = max(0.0, dot(v_normal, normalize(lightDir + viewVector)));
						v_lightSpecular += (value * pow(halfDotView, u_shininess)) * (1.0 - (1.0 - spotFactor) * 1.0 / (1.0 - u_spotLights[i].cutoffAngle));
					#endif // specularFlag
				}
			}
		#endif //numSpotLights
